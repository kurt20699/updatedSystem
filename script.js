...
function setupAuthHandlers() {
    ...
    loginForm?.addEventListener('submit', async (event) => {
        ...
        // ✅ Visible loading state so a slow connection doesn't invite a
        // double-click — restored in `finally` below regardless of which
        // path (success, server error, or network failure) this takes.
        const loginSubmitBtn = document.getElementById('loginSubmitBtn');
        if (loginSubmitBtn) {
            loginSubmitBtn.disabled = true;
            loginSubmitBtn.innerHTML = '<span class="auth-spinner"></span> Logging in…';
        }
...
function togglePassword(inputId, btn) {
    ...
    // ✅ innerHTML above only replaces the icon, not the button's own
    // attributes, so aria-label has to be updated separately here to stay
    // truthful about what the button currently does.
